...
export default defineEventHandler(async (event) => {
  ...
  if (session.data.acc.username === "admin") {
    if ("stream_id" in params && "app" in params && "ban" in params) {
      $fetch(
        `http://localhost/control/drop/publisher?app=${params.app}&name=${params.stream_id}`
      ).then(async (res) => {
        if (res === 1) {
          if (params.ban === "true" && params.app !== "live") {
            const updateRes = await useDrizzle()
              ...
              .set({
                stream_id: Array.from(crypto.getRandomValues(new Uint32Array(8)))
                .map((x) => characters[x % characters.length])
                .join(""),
                stream_key: Array.from(crypto.getRandomValues(new Uint32Array(20)))
                .map((x) => characters[x % characters.length])
                .join(""),
              })
