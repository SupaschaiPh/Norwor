  ...
  <div class="flex justify-between items-center p-3 bg-orange-50">
    ...
    <v-dialog max-width="1000" >
      <template v-slot:activator="{ props: activator }">
        ...
        <v-btn rounded="lg" v-bind="activator" flat prepend-icon="mdi-upload" color="primary"
      >Upload</v-btn
    >
      </template>

      <template v-slot:default="{ isActive }">
        <v-card v-if="file==null" rounded="lg" title="Upload new video" :loading="onUpload" >
          <v-card-text>
            <div class="h-64 w-full flex justify-center items-center">
                <NuxtImg src="/upload.jpeg" />
        </div>
            <v-file-input v-model="file" density="compact"  accept="video/" label="File input" variant="solo"></v-file-input>
          </v-card-text>
        </v-card>
        <v-card v-if="file!=null" rounded="lg" title="Upload new video" :loading="onUpload" >
          <v-card-text>
            <div class="flex w-full gap-6">
              <span class="w-4/12 overflow-hidden">
                ...
                <v-file-input @change="()=>previewCoverHandler()" v-model="cover" accept="image" label="File input" density="compact"  variant="solo"></v-file-input>
              </span>
              <span class="w-8/12">
            <v-text-field label="Title" variant="outlined" density="compact" ></v-text-field>
            <v-textarea rows="6" label="Description" variant="outlined" density="compact"></v-textarea>
          </span>
          </div>
          </v-card-text>
        <v-card-actions>
    ...
    <v-window v-model="tab">
      <v-window-item value="all">
        ...
        <template v-slot:item.action="{ item }">
          <v-btn rounded="lg" color="primary" flat variant="tonal" :href="'/me/edit/v/'+item.id">Edit</v-btn>
    </template>
        </v-data-table>
      </v-window-item>
